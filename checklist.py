...
def _listar_arquivos_pasta(pasta: str) -> list:
    """Lista (em ordem alfabética) os arquivos dentro de `pasta`, ignorando
    subpastas e arquivos ocultos. Devolve [] se a pasta não existir."""
    if not os.path.isdir(pasta):
        return []
    return sorted(
        os.path.join(pasta, nome)
        for nome in os.listdir(pasta)
        if os.path.isfile(os.path.join(pasta, nome)) and not nome.startswith(".")
    )


def localizar_anexos(pasta_anexos: str, candidatos: list) -> tuple:
    """
    ...
    main.py (só que restrita aos documentos deste fluxo).
    ...
    for caminho in _listar_arquivos_pasta(pasta_anexos):
    ...
    if tipo == "pdf":
        ...
        if precisa_conferir_assinatura:
            ...
            if assinatura.get("assinado"):
                ...
                print(f"    [OK] Assinatura: documento assinado (sistema: {sistema})")
                for s in assinatura.get("signatarios", []):
                    print(f"         - {s['nome']} ({s['cargo']}) em {s['data']} {s['hora']}")
            else:
                print("    [PENDENTE] Assinatura: nenhum indício de assinatura eletrônica encontrado")
...
def executar_checklist(id_fluxo: str, pasta_anexos: str = PASTA_ANEXOS_PADRAO) -> None:
    """
    ...
    um — em vez de só listar os campos esperados.
    """
    ...
    encontrados, nao_identificados = localizar_anexos(pasta_anexos, documentos_do_fluxo)
...
def main():
    if len(sys.argv) not in (2, 3):
        print("Uso: python checklist.py caminho/do/checklist.docx [pasta/de/anexos]")
    ...
    executar_checklist(id_fluxo, pasta_anexos)
